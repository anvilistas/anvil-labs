...
`;function M(){let e={resolve:()=>{},reject:()=>{}};return e.promise=new Promise((t,r)=>{e.resolve=t,e.reject=r}),e}var{misceval:{callsimArray:$,buildClass:O},ffi:{toPy:E},builtin:{RuntimeError:S,str:R,print:j}}=Sk,w=O({__name__:"anvil_labs.web_worker"},()=>{},"WorkerTaskKilled",[S]);function x(e,t,r){let s;if(e==="WorkerTaskKilled")return s=$(w,t),s.traceback=r,s;try{let a=Sk.builtin[e];s=$(a,t.map(n=>E(n))),s.traceback=r??[]}catch{let a;try{a=new window[e](...t)}catch{a=new Error(...t)}s=new Sk.builtin.ExternalError(a)}return s}function L(e){let t={};e.currentTask=null,e.stateHandler=()=>{},e.launchTask=(r,s,a)=>{let n=crypto.randomUUID();return e.currentTask={fn:r,id:n},e.postMessage({type:"CALL",id:n,fn:r,args:s,kws:a}),t[n]=M(),[n,r,new Promise(c=>c(t[n].promise))]},e.onmessage=async({data:r})=>{switch(r.type){case"OUT":{let{id:s,fn:a}=e.currentTask??{};j([`<worker-task '${a}' (${s})>:`,r.message],["end",R.$empty]);break}case"STATE":{e.stateHandler({...r.state});break}case"RESPONSE":{let{id:s,value:a,errorType:n,errorArgs:c,errorTb:o}=r,i=t[r.id];i?n?(console.debug(`RPC error response ${s}:`,n,c),i.reject(x(n,c,o))):(console.debug(`RPC response ${s}:`,a),i.resolve(a)):console.warn(`Got worker response for invalid call ${r.id}`,r),delete t[r.id],e.currentTask=null;return}case"IMPORT":{let{id:s,filename:a}=r,n=null;try{n=Sk.read(a),typeof n!="string"&&(n=await Sk.misceval.asyncToPromise(()=>n))}catch{if(a.startsWith("app/")){let[c,o,i]=a.split("/");i==="__init__.py"&&o!=="anvil"&&(n="pass")}else n=null}e.postMessage({type:"MODULE",id:s,content:n})}}}}var v=class{constructor(t,r,s,a){this._id=t;this._name=r;this._result=s;this._target=a;this._startTime=Date.now(),this._handledResult=s,this._target.stateHandler=this._updateState.bind(this),this._result.then(n=>{this._complete=!0,this._status="completed",this._rv=n},n=>{this._complete=!0,n instanceof w?(this._status="killed",this._err=new S("'"+this._name+"' worker task was killed")):(this._status="failed",this._err=n)})}_handledResult;_startTime;_state={};_rv=null;_err=null;_complete=!1;_status=null;_stateHandler=()=>{};_updateState(t){this._state=t,this._stateHandler(t)}await_result(){return this._result}on_result(t,r){this._handledResult=this._result.then(t),r&&(this._handledResult=this._handledResult.catch(r))}on_error(t){this._handledResult=this._handledResult.catch(t)}on_state_change(t){this._stateHandler=t}get_state(){return this._state}get_id(){return this._id}get_task_name(){return this._name}get_termination_status(){return this._status}get_return_value(){if(this._err)throw this._err;return this._rv}get_error(){if(this._err!==null)throw this._err}get_start_time(){return this._startTime}is_completed(){if(this._err)throw this._err;return this._complete}is_running(){return!this._complete}kill(){this._complete||this._target.postMessage({type:"KILL",id:this._id})}},g=class{target;constructor(t){t.startsWith(window.anvilAppMainPackage)||(t=window.anvilAppMainPackage+"."+t);let r=A.replace("{$filename$}",JSON.stringify(t)).replace("self.anvilAppOrigin",JSON.stringify(window.anvilAppOrigin)),s=new Blob([r],{type:"text/javascript"});this.target=new Worker(URL.createObjectURL(s)),L(this.target)}launch_task(t,r,s){if(this.target.currentTask!==null)throw new S("BackgroundWorker already has an active task");let[n,c,o]=this.target.launchTask(t,r,s);return new v(n,c,o,this.target)}};window.anvilLabs??={};window.anvilLabs.Worker=g;window.anvilLabs.WorkerTaskKilled=w;})();
"""
